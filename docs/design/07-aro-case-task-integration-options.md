# Design Pattern 07: ARO case/task management integration
...
*This diagram shows Option A's shape: ARO stays authoritative for the Verkaufschance, CRM holds only a read-only Opportunity projection fed via Kafka, and advisors cross-launch to ARO's own UI to make any edit.*
...
*This diagram shows Option B's shape: CRM's Opportunity becomes the system of record, ARO's Verkaufschancen capability is decommissioned, and a won Opportunity publishes an event that triggers contract creation in Versicherungsprozesse.*
...
*This diagram shows Option C's shape: a bidirectional reconciliation layer keeps ARO and CRM Opportunities in sync while GAs cut over incrementally, until every GA has migrated and the reconciliation layer retires.*
