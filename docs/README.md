# Documentation Index — CRM Frontier Firm Showcase

This folder holds product, design, AI, security, compliance, and test artefacts
that the Copilot agents in [.github/agents/](../.github/agents/) reason over.

## Start here

- [DESIGN-PRINCIPLES.md](./DESIGN-PRINCIPLES.md) — read before making any design decision.
- [PRD.md](./PRD.md) — product intent, use cases, backlog.
- [PERSONAS-JOURNEY.md](./PERSONAS-JOURNEY.md) — who uses the showcase and how.
- [COPILOT-BUILD-GUIDE.md](./COPILOT-BUILD-GUIDE.md) — how PRs move through this repo.

## Cross-cutting

- [AI.md](./AI.md) — model choices, prompts, evals, Content Safety.
- [DATA.md](./DATA.md) — data model, synthetic data rules.
- [SECURITY.md](./SECURITY.md) — identity, secrets, network posture.
- [COMPLIANCE.md](./COMPLIANCE.md) — what claims the demo may and may not make.
- [TEST.md](./TEST.md) — testing strategy.
- [GLOSSARY.md](./GLOSSARY.md) — shared vocabulary.

## Decision records

- [adr/](./adr/) — one Architecture Decision Record per architectural choice.
