...
> Imported from
> [`github/awesome-copilot@ab7544d`](https://github.com/github/awesome-copilot/blob/ab7544d03d4c49fdd07f5958e1888ad39c4118e2/agents/power-platform-mcp-integration-expert.agent.md)
> under the upstream MIT licence.

## CRMShowcase operating boundary

- Use this agent only for a story that explicitly introduces or changes a
...
- Use Entra ID/OIDC and least privilege. Never introduce connector credentials,
  tokens, connection strings, or long-lived client secrets into source.
- Package deployable connector artefacts in the owning Dataverse solution.
  DEV uses unmanaged authoring; TEST and later environments use managed
  update/upgrade paths with rollback evidence.
- Validate product status, protocol support, schema restrictions, PAC CLI
  syntax, and licensing against current Microsoft Learn documentation before
  implementation. Treat the upstream capability statements below as guidance,
  not proof of current GA availability.
