...
description: 'Use when creating or restructuring a Power Apps Code App with React, TypeScript, Vite, Power Apps CLI, Power Apps SDK, connectors, local development, testing, or deployment.'
---

# Power Apps Code Apps Project Scaffolding

> Imported from
> [`github/awesome-copilot@ab7544d`](https://github.com/github/awesome-copilot/blob/ab7544d03d4c49fdd07f5958e1888ad39c4118e2/skills/power-apps-code-app-scaffold/SKILL.md)
> and updated to the current upstream Code Apps guidance. CRMShowcase requires
> Entra authentication, synthetic data, EN/DE/FR/IT localization, human
> approval for customer-impacting actions, and story/ADR/test evidence.

You are an expert Power Platform developer who specializes in creating Power Apps Code Apps. Your task is to scaffold a complete Power Apps Code App project following Microsoft's current best practices.

## Context

Power Apps Code Apps allow developers to build custom web applications using code-first approaches while integrating with Power Platform capabilities. These apps can access 1,500+ connectors, use Microsoft Entra authentication, and run on managed Power Platform infrastructure.
...
- Install and configure the Power Apps SDK (`@microsoft/power-apps ^1.2.5`)
- Install and configure the Vite plugin (`@microsoft/power-apps-vite ^1.0.2`)
- Install `@microsoft/power-apps-cli` and initialize with `pa app init`

### 2. Essential Configuration Files
- **vite.config.ts**: Register React and `powerApps()` plugins
- **power.config.json**: Generated by Power Apps CLI for Power Platform metadata
...
- `dev`: "vite" for the template development server
...
Use `pa app run` as the Power Apps local-host command. Do not run it
concurrently with a second Vite server.
...
- PowerProvider pass-through wrapper; SDK v1.0 requires no async initialization
...
- Node.js (a currently supported LTS version)
- Git for version control
- Power Apps CLI (`@microsoft/power-apps-cli`) - latest compatible version
...
### Power Apps CLI Commands to Include
- `pa auth login` - Authenticate and select an account
- `pa auth status` - Verify the active account
- `pa app init --display-name "App Name" --environment-id {environment-id}` - Initialize the code app
- `pa connection list` - List available connections
- `pa app add data-source --connector {connector-id} --connection-id {connection-id}` - Add a connector
- `pa app push --solution-id {solution-id}` - Publish into the target solution
...
- Application Insights is available through SDK logger configuration rather than built-in native instrumentation

### Best Practices to Include
- Set `verbatimModuleSyntax: false` in TypeScript config
- Configure `vite.config.ts` with `react()` and `powerApps()` from
...
- Use generated TypeScript models and services from Power Apps CLI
- Keep PowerProvider as a pass-through wrapper for SDK v1.0
...
Ensure the generated project follows Microsoft's official
[Power Apps CLI guidance](https://learn.microsoft.com/power-apps/developer/code-apps/reference/cli)
and samples from https://github.com/microsoft/PowerAppsCodeApps, and can be
successfully published using `pa app push`. Treat `pac code` as legacy
guidance only; do not generate new projects with it.
